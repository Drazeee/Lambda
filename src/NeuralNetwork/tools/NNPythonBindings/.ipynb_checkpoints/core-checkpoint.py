...
LoadDatasetOrig.argtypes = [c_char_p, c_int, c_int, POINTER(s.MMContext)]
LoadDatasetOrig.restype = POINTER(s.MMImage)

LoadDataset = lambda s, a, b, c: LoadDatasetOrig(s.encode('utf-8'), a, b, c)
...
SaveNetworkOrig = lib.SaveNetwork
SaveNetworkOrig.argtypes = [s.MMNetwork, c_char_p]

SaveNetwork = lambda n, s: SaveNetworkOrig(n, s.encode('utf-8'))
